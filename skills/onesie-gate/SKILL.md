...
A gate with a cheap yes, a cheap no and an expensive middle, usually a person, needs a third outcome. `--abstain-if` is a second expression in the `--assert` language and needs `--assert`. The assertion is checked first: when it holds the record is a yes and exits 0. When it does not, the abstain expression is checked: when that holds the record is an unsure and exits 7, otherwise a no and exits 1. onesie fixes no rule for several questions, so you write it. For a tool guard, deny wins: assert that every danger is low, and abstain when none is high, so one high answer is a no and anything in between asks the user. Branch with `case $? in 0) eval "$cmd" ;; 1) echo blocked ;; 7) ask the user ;; *) alert and block ;; esac`. Without `--abstain-if` the middle ground exits 1 like any other no.
