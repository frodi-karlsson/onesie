...
name: onesie-gate
description: Use when a onesie answer decides whether a command runs, a step continues, a build fails or a low confidence answer goes to a human, and before writing 'if onesie', 'onesie ... && cmd', -q or --assert, or picking the cut a gate uses. One common gate recipe looks right and is silently wrong.
...
### Calibrate the question against labelled records before you pick a --threshold or an --assert cut.

A cut picked by feel says nothing about what it catches or lets through. `onesie calibrate` asks the question about records whose right answer a `--label` jq expression takes from each record, and prints one row per cut with its catches, false alarms and right when flagged, each with a 95 percent interval. A row flags a record when `value >= cut`, the test `--threshold` and `--assert 'urgent.value >= 0.9'` apply, so take the cut of the row you can live with as written. calibrate never picks the cut. Map only the text a person would read, since a `--map` that selects the label flatters the question. Add `--out answers.jsonl --resume` with `--id`, so a rerun, a fixed label or new `--cuts` asks nothing again. A plain stream run can resume that file too, given the same questions, model, `-i`, `--map` and `--id`, with `-o json` and no gate or merge. calibrate ignores a question file's `assert`, `abstain_if`, `threshold`, `min_confidence` and `fallback`, so a gated file calibrates as it stands, while the same flags exit 2. Add `--require 'urgent.catches >= 0.95 at 0.9'` to turn the report into a check that exits 1 when the requirement does not hold. Without `at`, the cut comes from the `-f` file's `assert`. In CI, commit the answers file and its `.onesie` sidecar and add `--offline`, which reads every answer from the file and asks nothing, so the check needs no key and costs nothing. Pin `-m` and `--provider` there, since the file only matches the model it came from. `onesie calibrate --help` has the rest.

**Bad:**

```sh
onesie calibrate --ask urgent='is this urgent' -i jsonl --map '{body, is_urgent}' --label urgent='.is_urgent' --id '.id'
```

**Good:**

```sh
onesie calibrate --ask urgent='is this urgent' -i jsonl --map '.body' --label urgent='.is_urgent' --id '.id' --require 'urgent.catches >= 0.95 at 0.9'
```
...
The assertion language has four functions over numbers, `min`, `max`, `sum` and `avg`. Each takes one or more numbers, such as `d.value`, `t.confidence`, `t.p.billing`, a literal or another call, so they nest as in `max(min(a.value, b.value), c.value)`. A chain of `and` that holds every risk under one bound is one `max`: the chain is not wrong, only longer and easier to get out of step when a question is added. `==` rarely matches the result of `sum` or `avg`, since sums of fractions are inexact, so compare them with `>=` or `<=`. There are no arithmetic operators, so weighting answers still belongs in `jq`, as in `onesie --ask urgency='how urgent is this' --ask anger='how angry is the writer' --state "$t" -o values | jq '(.urgency.value * 2 + .anger.value) / 3'`. So does anything over a whole probability map, such as the top two options of `p`.
