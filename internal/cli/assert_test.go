...
func TestGateOf(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "should report a type error in --abstain-if before any request",
			file:      "assert: urgent.value > 0.5\n" + question,
			args:      []string{"--abstain-if", `urgent.value > "hot"`},
			wantCode:  cli.ExitUsage,
			wantErr:   `onesie: --abstain-if: cannot compare 'urgent.value', a number, with "hot", a string`,
			noRequest: true,
		},
		{
			name:      "should name --abstain-if when the flag joins the file's key with no assertion anywhere",
			file:      "abstain_if: urgent.value > 0.5\n" + question,
			args:      []string{"--abstain-if", "urgent.value < 0.95"},
			wantCode:  cli.ExitUsage,
			wantErr:   "onesie: --abstain-if needs --assert, since without one every record is a yes",
			noRequest: true,
		},
		{
			name:     "should accept the file's abstain expression beside an assertion given only by --assert",
			file:     "abstain_if: urgent.value > 0.5\n" + question,
			args:     []string{"-o", "json", "--assert", "urgent.value > 0.5"},
			wantCode: cli.ExitOK,
			contains: []string{`"urgent":{"value":0.9}`},
			missing:  []string{"assert", "abstain"},
		},
		{
