...
func TestWriteTable(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "should add an other answers column to the grid when an answer falls outside the options",
			...
			want: []string{
				...
				"",
				`  labelled \ picked  billing  shipping  other answers`,
				"  billing                  1         0              1",
				"  shipping                 0         0              0",
				"",
				"  confidence  answered          agreement",
		...
		{
			name: "should align names by display width, counting a wide character as two",
			questions: []calibrate.QuestionReport{pickReport("team", calibrate.ScorePick(
				[]calibrate.ChoiceCase{
					{Name: "Å-1", ID: "Å-1", Line: 1, Label: "café", Picked: "café", Confidence: 0.9},
					{Name: "Å-2", ID: "Å-2", Line: 2, Label: "技术", Picked: "café", Confidence: 0.8},
					{Name: "Å-3", ID: "Å-3", Line: 3, Label: "other", Picked: "refunds", Confidence: 0.7},
					{Name: "Å-4", ID: "Å-4", Line: 4, Label: "技术", Picked: "技术", Confidence: 0.6},
				},
				[]string{"café", "技术", "other"}, 0, []float64{0.5},
			))},
			want: []string{
				"team, pick: labelled 4, 0 failed. agreement 50% 15-85%",
				"",
				"  option  labelled  picked  found             right when picked",
				"  café           1       2  1/1 100% 21-100%  1/2  50%  9-91%",
				"  技术           2       1  1/2  50%  9-91%   1/1 100% 21-100%",
				"  other          1       0  0/1   0%  0-79%   0/0    -",
				"",
				`  labelled \ picked  café  技术  other  other answers`,
				"  café                  1     0      0              0",
				"  技术                  1     1      0              0",
				"  other                 0     0      0              1",
				"",
				...
				"",
				"worst misses",
				"  Å-2  labelled 技术   picked café     confidence 0.80",
				"  Å-3  labelled other  picked refunds  confidence 0.70",
			},
		},
		{
			name: "should give the other column a heading no option uses",
			questions: []calibrate.QuestionReport{pickReport("team", calibrate.ScorePick(
				namedChoice("billing", "billing", 0.9, "other", "refunds", 0.8),
				[]string{"billing", "other"}, 0, []float64{0.5},
			))},
			want: []string{
				"team, pick: labelled 2, 0 failed. agreement 50% 9-91%",
				"",
				"  option   labelled  picked  found             right when picked",
				"  billing         1       1  1/1 100% 21-100%  1/1 100% 21-100%",
				"  other           1       0  0/1   0%  0-79%   0/0    -",
				"",
				`  labelled \ picked  billing  other  other answers`,
				"  billing                  1      0              0",
				"  other                    0      0              1",
				"",
				"  confidence  answered          agreement",
				...
				"",
				"worst misses",
				"  T-2  labelled other  picked refunds  confidence 0.80",
			},
		},
		{
			name: "should add a suffix to the other heading when an option already uses it",
			questions: []calibrate.QuestionReport{pickReport("team", calibrate.ScorePick(
				namedChoice("billing", "refunds", 0.8),
				[]string{"billing", "other answers"}, 0, []float64{0.5},
			))},
			want: []string{
				"team, pick: labelled 1, 0 failed. agreement 0% 0-79%",
				"",
				"  option         labelled  picked  found         right when picked",
				"  billing               1       0  0/1 0% 0-79%  0/0 -",
				"  other answers         0       0  0/0  -        0/0 -",
				"",
				`  labelled \ picked  billing  other answers  other answers 2`,
				"  billing                  0              0                1",
				"  other answers            0              0                0",
				"",
				"  confidence  answered          agreement",
				"  0.50        1/1 100% 21-100%  0/1 0% 0-79%",
				"",
				"worst misses",
				"  T-1  labelled billing  picked refunds  confidence 0.80",
			},
		},
		{
			name: "should quote an id or a question id that holds a newline or an escape, and print a miss value in full",
			questions: []calibrate.QuestionReport{yesNoReport("urgent\nfake", calibrate.ScoreYesNo(
				[]calibrate.YesNoCase{
					{Name: "T\n1", ID: "T\n1", Line: 1, Yes: true, Value: 0.1},
					{Name: "\x1b[31mT-2", ID: "\x1b[31mT-2", Line: 2, Yes: false, Value: 0.503},
				},
				0, []float64{0.5},
			))},
			want: []string{
				`"urgent\nfake", yes/no: labelled 2, 1 yes, 1 no, 0 failed. AUC 0.00`,
				`flagged means "urgent\nfake".value >= cut`,
				"",
				"  cut   flagged  catches       false alarms      right when flagged",
				"  0.50        1  0/1 0% 0-79%  1/1 100% 21-100%  0/1 0% 0-79%",
				"",
				"worst misses",
				`  "T\n1"         labelled yes  answered 0.10`,
				`  "\x1b[31mT-2"  labelled no   answered 0.503`,
			},
		},
		{
			name: "should quote an option name that holds a newline",
			questions: []calibrate.QuestionReport{pickReport("team", calibrate.ScorePick(
				namedChoice("a\nb", "a\nb", 0.9),
				[]string{"a\nb"}, 0, []float64{0.5},
			))},
			want: []string{
				"team, pick: labelled 1, 0 failed. agreement 100% 21-100%",
				"",
				"  option  labelled  picked  found             right when picked",
				`  "a\nb"         1       1  1/1 100% 21-100%  1/1 100% 21-100%`,
				"",
				`  labelled \ picked  "a\nb"`,
				`  "a\nb"                  1`,
				"",
				"  confidence  answered          agreement",
				"  0.50        1/1 100% 21-100%  1/1 100% 21-100%",
			},
		},
		{
			name: "should print 100 only for all hits and 0 only for none, bounds included",
			questions: []calibrate.QuestionReport{yesNoReport("urgent", calibrate.YesNoScore{
				Labelled: 2000, Yes: 1000, No: 1000, AUC: 0.5, HasAUC: true,
				Cuts: []calibrate.CutRow{
					{
						Cut: 0.5, Flagged: 999, Catches: calibrate.Wilson(999, 1000),
						FalseAlarms: calibrate.Wilson(1, 1000), RightWhenFlagged: calibrate.Wilson(999, 999),
					},
					{
						Cut: 0.9, Flagged: 0, Catches: calibrate.Wilson(0, 1000),
						FalseAlarms: calibrate.Wilson(1, 1000), RightWhenFlagged: calibrate.Wilson(999, 999),
					},
				},
				Misses: []calibrate.YesNoCase{},
			})},
			want: []string{
				"urgent, yes/no: labelled 2000, 1000 yes, 1000 no, 0 failed. AUC 0.50",
				"flagged means urgent.value >= cut",
				"",
				"  cut   flagged  catches              false alarms    right when flagged",
				"  0.50      999  999/1000 99% 99-99%  1/1000 1% 1-1%  999/999 100% 99-100%",
				"  0.90        0    0/1000  0%  0-1%   1/1000 1% 1-1%  999/999 100% 99-100%",
			},
		},
		{
