...
For maintainers. [CONTRIBUTING.md](CONTRIBUTING.md) covers issues and pull requests.

## Development

`go.mod` names the Go version. CI reads it through `go-version-file`, so there is one number to
bump.
