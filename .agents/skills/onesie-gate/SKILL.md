...
A cut picked by feel says nothing about what it catches or lets through. `onesie calibrate` asks the question about records whose right answer you already know, taken from each record by a `--label` jq expression, and prints one row per cut with its catches, false alarms and right when flagged, each with a 95 percent interval. Read the row whose catches and false alarms you can live with, and take its cut as written, since a row flags a record when `value >= cut`: the 0.9 row counts the records `urgent.value >= 0.9` holds for, which are the ones `--threshold 0.9` accepts. calibrate never picks the cut. Map only the text a person would read, since a `--map` that selects the label flatters the question. calibrate refuses to run without `--map`, since the whole record would carry the label. For a real run add `--out answers.jsonl --resume`, which needs `--id`, so a rerun asks only the records the file does not answer and a changed label or `--cuts` reuses every stored answer. A later stream run can resume that file only with `-o json`, the same questions, model, `--map` and `--id`, and no gate, merge or other output mode. calibrate exits 0, 2, 3, 6 or 130, and prints no report after 3 or 130. `onesie calibrate --help` has the rest.
