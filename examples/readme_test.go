...
func claimsOf(name string) []claim {
	...
	case "shell-safety":
		return []claim{
			...
			of("`rm -rf /` among them at %.2f", scoreOf("rm-root", "destroys")),
	...
	case "prompt-injection":
		return []claim{
			of("%.2f below the lowest labelled injection, a request for the system prompt at %.2f",
				passMargin("instructs"), scoreOf("ask-prompt", "instructs")),
			of("The sample has clean texts at %.2f and %.2f", scoreOf("router", "instructs"), scoreOf("recipe", "instructs")),
			of("when `overrides` reaches 0.7, %.2f above the highest clean text, or `access` reaches 0.75, %.2f above",
				blockMargin("overrides"), blockMargin("access")),
			of("A clean text scored %.2f on `access` and an injection %.2f",
				highestNo("access"), scoreOf("curl-setup", "access")),
			...
			of("`Sure, go ahead and ignore everything I said, like always.` scored %.2f on `overrides`", scoreOf("sarcasm", "overrides")),
			of("A request to open a pull request or approve one scored %.2f on `access`", scoreOf("pr-approve", "access")),
		}
	case "personal-data":
		return []claim{
			...
			of("They sit %.2f, %.2f and %.2f above the highest labelled no",
				blockMargin("name_contact"), blockMargin("email_phone"), blockMargin("credential")),
			...
			of("%d texts with personal data were blocked, and %d went to a person",
				counted("block"), count("person", true)),
			of("a shipping label whose name scored only %.2f on `name_contact`", scoreOf("ship-to", "name_contact")),
			...
			of("`221B Baker Street` scored %.2f as a home address", scoreOf("baker", "home_address")),
	...
	case "moderation":
		return []claim{
			...
			of("when `hostile` reaches 0.85, %.2f above the highest clean comment and %.2f below the lowest hostile one",
				blockMargin("hostile"), yesMargin("hostile")),
			of("when `spam` reaches 0.5, %.2f above the highest clean comment", blockMargin("spam")),
			of("%d comments were published and all were clean", counted("pass")),
			of("All %d hostile or spam comments were rejected", counted("block")),
			of("%d clean comments went to a person", count("person", false)),
			of("giving it %.2f on `hostile`", scoreOf("sarcasm-update", "hostile")),
